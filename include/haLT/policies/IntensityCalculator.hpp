...
#include "haLT/testLib/IntensityLibrary.hpp"
    ...
    private:
        ...
        struct ExecutionPolicy< 1, false, T_isAoS >
        {
            ...
            void operator()(Input& input, Output output){
                LibFoo::calculateR1D(
                        safe_ptr_cast<Output>(&m_acc(haLT::types::Vec<1>::all(0), input)),
                        output,
                        Extents(input)[0]);
            }
        };
        ...
        struct ExecutionPolicy< T_numDims, false, T_isAoS >
        {
            ...
            void operator()(Input& input, Output output){
                ...
                LibFoo::calculateRND(safe_ptr_cast<Output>(&m_acc(haLT::types::Vec<T_numDims>::all(0), input)),
        ...
        struct ExecutionPolicy< T_numDims, true, true, DUMMY >
        {
            ...
            void operator()(Input& input, Output output){
                ExtentsPtr extents(input);
                LibFoo::calculateCND(safe_ptr_cast<Output>(&m_acc(haLT::types::Vec<T_numDims>::all(0), input)),
                        output, T_numDims, extents());
            }
        };
        ...
        struct ExecutionPolicy< 1, true, false, DUMMY >
        {
            T_Accessor m_acc;
            void operator()(Input& input, Output output){
                auto ptr = m_acc(haLT::types::Vec<1>::all(0), input);
        ...
        struct ExecutionPolicy< T_numDims, true, false, DUMMY >
        {
            ...
            void operator()(Input& input, Output output){
                ExtentsPtr extents(input);
                auto ptr = m_acc(haLT::types::Vec<T_numDims>::all(0), input);
