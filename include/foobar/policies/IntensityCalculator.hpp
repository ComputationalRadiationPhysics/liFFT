...
namespace policies {
    ...
    public:
        using Input = T_Input;
        using Memory = typename traits::MemoryType<Input>::type;
        using Output = typename traits::IntegralType<Memory>::type*;
        using RawPtr = GetRawPtr<Input>;
        using Extents = GetExtents<Input>;
        using ExtentsPtr = GetExtentsRawPtr<Input>;
        static constexpr bool isComplex = traits::IsComplex<Memory>::value;
        static constexpr unsigned numDims = traits::NumDims<Input>::value;
        static_assert(numDims >= 1, "Need >= 1 dimension");
        static constexpr bool isAoS = traits::IsAoS<Memory>::value;
    ...
    void
    useIntensityCalculator(const T_Input& input, Output output){
